import { fetchBattingLeaderboard } from "@/services/battinginfoService";
...
const BattingLeaderboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const data = await fetchBattingLeaderboard();
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          ...
          <tr>
            ...
            <th
              className="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase border-l border-gray-400 cursor-pointer"
              onClick={() => handleSort("ninety_five_plus")}
            >
              95+ MPH
              {sortConfig.key === "ninety_five_plus" &&
                (sortConfig.direction === "asc" ? "↑" : "↓")}
            </th>
            <th
              className="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase cursor-pointer"
            ...
            <tr key={player.player_id} className="hover:bg-gray-50">
              ...
              <td className="px-3 py-2 whitespace-nowrap border-b border-t border-gray-400">
                <div className="flex items-center">
                  ...
                  <div className="ml-2 mr-7 text-sm text-black">
                    {player.name}
                  </div>
                </div>
              </td>
